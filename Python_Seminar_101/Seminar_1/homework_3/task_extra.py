...
def create_polinom(n):  # Создаем строку-полином
    ...
    for i in range(1, n + 1):
        sign = ' + ' if random.randint(0, 1) else ' - '  # закладываю пробелы до и после знаков чтобы потом сплитовать (можно доделать чтобы без пробелов было)
...
def parse_to_dic(polynom):  # разбиваем строку в словарь, где ключом будут 'х**[]'
    ...
    for i in range(len(polynom_list)):  # проходим по списку и все элементы с х разбиваем на ключ 'x**[]' и значение - коэффициент
        if 'x' in polynom_list[i]:
            num_str = ''
            while '0' <= polynom_list[i][0] <= '9':
                num_str += polynom_list[i][0]
                polynom_list[i] = polynom_list[i][1:]
            polynom_dic[polynom_list[i]] = int(polynom_list[i - 1] + num_str) # вносим значение под соответствующим ключом, учитывая знак перед элементом
    return polynom_dic
...
def sum_polynoms(polynom1_dic, polynom2_dic):  #
    ...
    result = str(result_dic['0']) if result_dic['0'] != 0 else ''  # создание строки-результата и добавление в нее значения для ключа '0'
